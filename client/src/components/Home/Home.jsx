...
import { getDogs, filterDogsByTemps, getTemperaments, filterApi, orderWeight, orderAlfab } from "../../actions";
...
export default function Home() {
    ...
    return (
        <div className={style.div}>
            <button onClick={e => handlerClick(e)} className={style.b}>Volver a cargar TODOS los perros</button>
            {allDogs.length > 0
                ? <div>
                    <div className={style.SearchLink}>
                        <SearchBar setCurrentPage={setCurrentPage} allDogs={allDogs} />
                        <Link to='/dog'> <button className={style.b}>Crear raza de perros</button></Link>
                    </div>
                    <div>
                        ...
                        <select onChange={e => handlerFilterByTemps(e)} className={style.b}>
                            ...
                            {temperaments?.map((temp) => {//muestro todos los temperamentos como opciones
                                return (
                        ...
                        <select onChange={e => handlerFilterApi(e)} className={style.b}>
                            ...
                            <option value='api'>Existenteen la api</option>
                        </select>
                        <Paginated
                        ...
                        <div className={style.cards}>
                            {currentDogs?.map(elem => {//tomo unicamente los dogs que me devuelve el paginado para mostrar
                                return (
                                    <div className={style.card} key={elem.id}>
                                        ...
                                        <Card
                                            name={elem.name}
                                            image={elem.image ? elem.image : 'https://st3.depositphotos.com/29384342/35239/v/450/depositphotos_352397770-stock-illustration-vector-image-dog-silhouette-default.jpg'}
