...
const validate = (input, dogs) => {
    ...
    if (!(/^[a-zA-Z]+$/).test(input.name)) {
        errors.name = 'Este dato es incorrecto... Es obligatorio, no se permiten caracteres especiales, números o espacios'
    }
    if (dogs.some(e => e.name.toUpperCase() === input.name.toUpperCase())) {
        errors.name = 'Esta raza de perro ya existe!'
    }
    if (!(/^[0-9]+$/).test(input.height)) {
        errors.height = 'Este dato es incorrecto... Es obligatorio, solo números decimales, positivos, sin caracteres especiales o LETRAS'
    }
    if (!(/^[0-9]+$/).test(input.weight)) {
        errors.weight = 'Este dato es incorrecto... Es obligatorio, solo números decimales, positivos, sin caracteres especiales o LETRAS'
    }
    if (!(/^[ 0-9-]+$/).test(input.life_span)) {
        errors.life_span = 'Este dato es incorrecto... No se permiten caracteres especiales o letras'
    }
    if (!(/^(?:([A-Za-z]+):)?(\/{0,3})([0-9.\-A-Za-z]+)(?::(\d+))?(?:\/([^?#]*))?(?:\?([^#]*))?(?:#(.*))?$/).test(input.image)) {
        errors.image = "Solo se permiten URL's. En caso de estar vacío se pondrá una imagen predeterminada."
    }
    return errors;
}
            ...
            <div className={style.form}>
                ...
                <form onSubmit={(e) => handlerSubmit(e)}>
                    <div className={style.info}>
                        <label>Nombre: </label>
                        <input type='text' name='name' value={input.name} onChange={(e) => handlerChange(e)} >
                        </input>
                        {errors.name && (
                    ...
                    <div className={style.info}>
                        ...
                        <input type='number' value={input.height} name='height'
                            ...
                            disabled={!input.name || errors.name}></input>
                    ...
                    <div className={style.info}>
                        ...
                        <input type='number' value={input.weight} name='weight'
                            onChange={(e) => handlerChange(e)}
                            disabled={!input.name || errors.height}></input>
                    ...
                    <div className={style.info}>
                        ...
                        <input type='text' value={input.life_span} name='life_span'
                            onChange={(e) => handlerChange(e)}
                            disabled={!input.name || errors.weight}></input>
                    ...
                    <div>
                        <button type="create" className={style.button}
                            disabled={!input.name || errors.name || errors.height || errors.weight}
                        >Crear raza!</button>
                    </div>
                </form>
                <div className={style.temps}>
                    {input.temperament.map(temp => //mapeo mi estado local con todas las ocupaciones que le está mandando el usuario
                        <div key={temp} className={style.temp}>
                            <p >{temp}</p>
                            <button onClick={() => handleDelete(temp)} className={style.x}>x</button>
                        </div>
                    )}
                </div>
            </div>
